...
export async function GET(request: NextRequest) {
  try {
    ...
    if (rows.length < 2) {
      return freshJson({
        ...
        metadata: {
          ...
          total: 0,
          returned: 0,
          source_file: WINDOW_TO_FILE[windowKey],
    ...
    const tradeRows = dataRows.map((line) => normalizeTradeRow(toRecord(headers, line)));
    const leaderRows = buildLeaderRows(tradeRows);
    const topMarkets = leaderRows.slice(0, limit).map((row) => ({
      ...row,
      _window: windowKey,
    ...
    console.error('Error fetching suspicious markets:', err);
    return freshJson(
      {
