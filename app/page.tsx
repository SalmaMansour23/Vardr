...
export default function LeakLensDashboard() {
  ...
  const activeContract = useMemo(
    () => allContracts[activeContractIndex],
    [allContracts, activeContractIndex]
  );
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold font-headline tracking-tight">
                DriftX{" "}
                <span className="text-primary text-xs ml-1 bg-primary/10 px-1.5 py-0.5 rounded">
                  v3.0
                </span>
              </h1>
        ...
        <div className="flex flex-col gap-8">
          <div className="flex items-center gap-4 overflow-x-auto pb-2 no-scrollbar">
            {allContracts.map((c, i) => (
              ...
              >
                <div className="text-[10px] uppercase tracking-tighter opacity-70 font-bold mb-1">
                  Contract {i + 1}
                </div>
          ...
          <Tabs defaultValue="dashboard" className="w-full">
            <div className="flex justify-between items-center mb-6">
              <TabsList className="bg-card/30 border border-border/50 p-1 rounded-xl">
                <TabsTrigger
                  value="dashboard"
                  className="gap-2 text-xs uppercase font-bold px-4 py-2 rounded-lg"
                >
              ...
              <div className="flex items-center gap-2 px-4 py-2 bg-card/20 border border-border/50 rounded-xl">
                ...
                <Input
                  ...
                  className="h-6 w-32 border-none bg-transparent text-[11px] focus-visible:ring-0 p-0"
                />
              </div>
            </div>
                ...
                <div className="lg:col-span-8 space-y-8">
                  <Card className="border-border/50 bg-card/30 overflow-hidden rounded-2xl shadow-xl">
                    <div className="p-6 flex items-center justify-between border-b border-border/50">
                      <div className="flex items-center gap-2">
                        ...
                        <div>
                          <h2 className="text-sm font-bold uppercase tracking-wider text-muted-foreground">
                            Information Propogation Analysis
                          </h2>
                      ...
                      <Badge
                        variant="outline"
                        className="text-[9px] uppercase border-accent/20 text-accent"
                      >
                        Real-time Feed
                      </Badge>
                    </div>
                    <CardContent className="pt-8">
                      <ProbabilityChart
                        data={activeContract.data}
                        announcementTime={activeContract.announcementTime}
                      />
                    </CardContent>
                  </Card>
                    ...
                    <Card className="border-destructive/30 bg-destructive/5 rounded-2xl border-dashed">
                      <CardContent className="p-6 flex items-start gap-4">
                        ...
                        <div className="space-y-2">
                          <h4 className="text-xs font-bold uppercase tracking-tighter text-destructive underline decoration-dotted">
                            Behavioral Alert: Trader_Atlas
                          </h4>
                          <p className="text-[10px] text-muted-foreground leading-relaxed">
                            Detected High Information Asymmetry Risk. Position
                            accumulation correlates 0.94 with subsequent
                            probability drift in the pre-disclosure window.
                          </p>
                          <Button
                            variant="destructive"
                            size="sm"
            ...
            >
              <LiveKalshiTrades
                ...
                marketName={
                  activeContract.kalshiTicker ? activeContract.name : undefined
                }
              />
            </TabsContent>
      ...
      <footer className="mt-auto border-t border-border bg-card/20 py-10 px-6">
        <div className="container mx-auto flex flex-col md:flex-row items-center justify-between gap-6">
          <p className="text-[10px] text-muted-foreground/60 max-w-2xl leading-relaxed uppercase tracking-widest font-bold">
            DriftX models structural information asymmetry risk using
            statistical drift detection, behavioral sequencing analysis,
            cross-event correlation modeling, and sentiment divergence
            monitoring.
          </p>
